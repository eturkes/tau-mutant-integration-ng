Continue this project (fresh session). Non-empty task below ⇒ your sole task: do exactly it, editing `.agent/roadmap.md` only if it directs you to. Empty ⇒ run the MODE from the roadmap's active milestone; no active milestone ⇒ enter the PLANNING direction gate.

Load `.agent/roadmap.md` (trajectory ledger + active-milestone detail), then `.agent/memory.md` (lessons + decisions); `CLAUDE.md` is auto-injected. Read only what the step implicates. Navigate via tokensave or LSP where available, else grep.
...
- NONE (roadmap has no active milestone) → PLANNING direction gate
...
PLANNING — select the direction when none is active; otherwise split unsplit scope into milestones, then plan only the next milestone.
- No active milestone ⇒ derive concise next-direction candidates from the roadmap, ask the user to choose, and leave repository state unchanged until selection.
