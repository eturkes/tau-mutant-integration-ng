...
- `modality_scatter_figures <- modality_logfc_scatter_data(pb_de_microglia, symbol_map, geomx_de, proteome_de_24m, phospho_de_24m)`
  ...
  with aggregate `|P301S - MAPTKI| >= 0.5`; role assignment is priority-ordered from narrow
  process buckets to a broad immune/inflammatory residual bucket, and each visible category
  label lists every retained scored feature.
