...
- `modality_scatter_figures <- modality_logfc_scatter_data(pb_de_microglia, symbol_map, geomx_de, proteome_de_24m, phospho_de_24m)`
  ...
  The phosphoproteome native heatmap excludes parent gene `Plcb1`, keeps the same effect direction as
  the top-ranked candidate, collapses exact duplicate log2 median-normalized profiles to the first
  ranked representative with `(n)` label suffixes, and leaves the paired volcano unfiltered.
