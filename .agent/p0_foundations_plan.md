...
Cross-cutting (both stacks): pin versions via **P3M dated snapshot** (binary CRAN+Bioc
~7-8x faster; **caveat: trixie binaries may not exist yet -> source-compile fallback
keeps pinning, loses speed; verify at S1**). Repro layers, NO Docker, NO bitwise
...
- `rv init` -> project library + `rproject.toml` + `rv.lock`; hand-wire Bioc **3.23**
  (R 4.6) repos (BioCsoft/ann/exp/workflows); P3M dated-snapshot repo (verify trixie
  binaries; else source). Pin core stack: Seurat/SeuratObject, limma, edgeR,
...
- Quarto: install pinned quarto-cli; `_quarto.yml` book skeleton (**exclude `rv/library`**).
...
- **Accept:** fresh `rv sync` + `uv sync` rebuild env; `library()` smoke-load of core
  stack passes; `quarto --version` ok; `tar_manifest()` clean; lockfiles committed.
...
- **Accept:** each io fn returns documented-shape object vs LIVE data (Rscript
  smoke-test); microglia subset built; dims + `table(genotype, batch)` = 4x4=16 ids.
...
- **Accept:** chapter renders clean (0 error / 0 warning), self-contained HTML; QC
  metrics within documented sane ranges (state ranges, no pre-privileged threshold).

### S5 - concrete quality gate + lock + close  [GATE-INDEPENDENT; light]
- Runnable gate: `tar_make()` clean (rebuilds DAG -> renders QC chapter) asserting 0
  error/0 warning; thin `scripts/check.sh` wraps `rv sync` + `uv sync` + `tar_make()`.
...
## Risks / caveats
- P3M trixie binaries may be absent -> long source compiles at S1 (pinning intact).
...
- rv: hand-wire Bioc 3.23 repos; `_quarto.yml` excludes `rv/library` (#332); no
  `rv remove` (hand-edit TOML); pre-1.0 (v0.22, Jun 2026) -> if it bites, fall back to renv.
